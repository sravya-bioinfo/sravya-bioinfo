About Me:-
I bridge computational tools with biological questions — from molecular docking and virtual screening to RNA-Seq transcriptomics and ML-based predictions. I hold an ORCID iD and have been awarded an institutional research grant for my bioinformatics work.

Incoming Research Intern at Metflux Research, Bangalore (2026)
Selected for University Research Honors Program
Ranked Top 10 in Bioinformatics branch — VFSTR
Preparing for GATE Biotechnology 2027
ORCID: 0009-0005-9466-2789


Skills:-
Bioinformatics & Research
RNA-Seq analysis · DEG & pathway analysis · In-silico drug discovery · Molecular docking · Protein structure visualization · Epitope prediction · Host-pathogen interaction studies · Genomics & Transcriptomics
Computational Tools
AutoDock · AutoDock Vina · PyMOL · BLAST · KEGG · STRING · IEDB · ClusPro · DESeq2
Programming & Data
Python (basic–intermediate) · R (basic) · Pandas · NumPy · Matplotlib · Scikit-learn
Wet Lab
DNA isolation & purification · Agarose gel electrophoresis · ELISA · Blood group identification · Centrifugation · Buffer preparation

Projects:-
1. RNA-Seq Transcriptomic Profiling of Chlorella vulgaris
View Repository
Analyzed gene expression changes in Chlorella vulgaris under environmental stress. Identified differentially expressed genes and stress adaptation pathways using DESeq2, KEGG, and BLAST. Interpreted the biological significance of transcriptomic shifts.
RNA-Seq DESeq2 KEGG BLAST Transcriptomics

2. In-Silico Drug Design for Colorectal Cancer
Academic project targeting the AKT1 protein through virtual screening and molecular docking. Analyzed binding interactions and docking scores to shortlist potential lead compounds for further validation.
AutoDock AutoDock Vina PyMOL STRING KEGG

3. Study of Porphyromonas gingivalis in Rheumatoid Arthritis
Conducted sequence analysis and epitope prediction for P. gingivalis proteins. Performed protein structure visualization and immune epitope analysis to study host-pathogen interactions.
BLAST IEDB PyMOL ClusPro Immunoinformatics

4. Fake News Detection using NLP
View Repository
Built an NLP pipeline to classify news articles as real or fake. Applied TF-IDF vectorization and trained a Logistic Regression model after text preprocessing (tokenization, stopword removal).
Python NLP TF-IDF Logistic Regression Scikit-learn

5. Water Quality Prediction using Machine Learning
Built and evaluated ML models to classify water potability from physicochemical features. Performed feature analysis and model optimization to improve prediction accuracy.
Python Pandas Scikit-learn Feature Engineering

Certifications:-

NPTEL (IIT Madras) — WEKA: Exploring Machine Learning for Healthcare
Deloitte (Forage) — Data Analytics Job Simulation
HackerRank — Python (Basic) Skill Certification


Achievements:-

Merit Certificate for Poster Presentation Excellence — VFSTR
Selected for University Research Honors Program
Awarded Institutional Research Grant for bioinformatics research
Ranked in Top 10 students of the Bioinformatics branch
Represented at 6 competitive levels in Fencing
1st place — Inter-club Cricket Competition
2nd place — Yoga Competition


Connect:-

LinkedIn: sravya-mulpuri-943655336
ORCID: 0009-0005-9466-2789
Email: sravyasivamulpuri@gmail.com
Email: 221FA14063@vignan.ac.in
